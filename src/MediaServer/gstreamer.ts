...
export class GStreamer {
  ...
  get _videoArgs () {
    ...
    const ssrc = video?.rtpParameters?.encodings?.length ? video.rtpParameters.encodings[0].ssrc : ''
    const VIDEO_CAPS = `application/x-rtp,media=(string)video,clock-rate=(int)${videoCodecInfo.clockRate},payload=(int)${videoCodecInfo.payloadType},encoding-name=(string)${videoCodecInfo.codecName.toUpperCase()},ssrc=(uint)${ssrc}`
  ...
  get _audioArgs() {
    ...
    const ssrc = audio?.rtpParameters?.encodings?.length ? audio.rtpParameters.encodings[0].ssrc : ''
    const AUDIO_CAPS = `application/x-rtp,media=(string)audio,clock-rate=(int)${audioCodecInfo.clockRate},payload=(int)${audioCodecInfo.payloadType},encoding-name=(string)${audioCodecInfo.codecName.toUpperCase()},ssrc=(uint)${ssrc}`;
